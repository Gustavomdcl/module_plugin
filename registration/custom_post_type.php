...
add_action( 'init', 'create_post_type_registration' );
function create_post_type_registration() {
	//variável que recebe um array para distinguir o genero e se a palavra está no plural ou singular
	$nome = array( 
		'singular'				=> 'Registro', // o valor singular recebe 'Registro'
		'plural'				=> 'Registros', // o valor plural recebe 'Registros'
		'genero'				=> 'o', // o valor genero recebe 'o'
		'slug'					=> 'registration' // o valor slug recebe 'registration'
	);
	...
	$labels = array(
		...
		'all_items' 			=> 'Tod'.$nome['genero'].'s '.$nome['genero'].'s '.$nome['plural'], // todos os registrations
	...
	// variável que determinar o que cada campo recebe, no caso a variável labels e também algumas condições que especifica o que iremos usar no registration
	$args = array(
		...
		'menu_icon' 			=> 'dashicons-id', // icone do modulo
		...
		'supports' 				=> array('title','custom-fields','revisions') // o que será suportado pelo post
	  );
